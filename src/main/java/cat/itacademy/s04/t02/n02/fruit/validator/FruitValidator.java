...
public class FruitValidator {
    ...
    public void validate (Fruit fruit) {
        if (fruit == null){
            throw new ValidationException ("Fruit cannot be null");
        }
        if (fruit.getName() == null || fruit.getName().isBlank()){
            throw new ValidationException("Fruit name cannot be empty");
        }
        ...
        if(fruit.getProvider().getId() == null || fruit.getProvider().getId().describeConstable().isEmpty()) {
